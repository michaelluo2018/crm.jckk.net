...
<div class="container">
	<div class="page-header">
		<h4><?php echo L('SMS_TPL_SETTING');?></h4>
	</div>
	...
	<div class="row">
		<form action="<?php echo U('sms/delete');?>" id="form1" method="post">
			<div class="span12">
				<p><div class="bulk-actions align-left">
					<a class="btn" href="javascript:void(0);" id="btn_delete"/><i class="icon-remove"></i> <?php echo L('DELETE');?></a> &nbsp; <a class="btn" id="sort_btn"><i class="icon-file"></i> <?php echo L('SAVE_ORDER');?></a>
					<div class="pull-right">
						<a href="javascript:void(0);" id="add" class="btn btn-primary"><i class="icon-plus"></i> <?php echo L('ADD_SMS_TPL');?></a>
					</div>
				</div></p>
			</div>
			<div class="span12">
				<table class="table table-hover" width="95%" border="0" cellspacing="1" cellpadding="0">
					<?php if(!empty($templateList)): ?><thead>
							<tr>
								...
								<th width="20%"><?php echo L('TPL_SUBJECT');?></th>
						...
						<tfoot>
							<tr>
								<td colspan="6">
									<div class="span8"><span style="color: rgb(243, 40, 12);"><?php echo L('SAVE_ORDER_TIPS');?></span></div>
								</td>
							</tr>
						</tfoot>
						<tbody>
							<?php if(is_array($templateList)): $i = 0; $__LIST__ = $templateList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
									...
									<td><a href="javascript:void(0);" rel="<?php echo ($vo['template_id']); ?>" class="edit"><?php echo L('EDIT');?></a></td>
...
<div class="hide" id="dialog-add" title="<?php echo L('ADD_STATUS');?>">loading...</div>
<div class="hide" id="dialog-edit" title="<?php echo L('EDIT_STATUS');?>">loading...</div>
<div class="hide" id="dialog-delete" title="<?php echo L('DELETE_STATUS');?>">loading...</div>
</div>
<script type="text/javascript">	
$("#dialog-add").dialog({
	autoOpen: false,
...
$(function(){
	$("#btn_delete").click(function(){
		if(confirm('<?php echo L("DELETE_CONFIRM");?>')){
			$('#form1').submit();
		}
	});
	...
	$("#add").click(function(){
		...
		$('#dialog-add').load('<?php echo U("sms/add");?>');
	});
	$(".edit").click(function(){
		var id = $(this).attr('rel');
		$('#dialog-edit').dialog('open');
		$('#dialog-edit').load('<?php echo U("sms/edit","id");?>'+id);
	});
	$("#sort_btn").click(
		function() {
			position = [];
			$.each($(".list"), function(i, item){position.push(item.value)});
			$.get('<?php echo U("sms/ordersort");?>',{postion:position.join(',')}, function(data){
				if (data.status == 1) {
