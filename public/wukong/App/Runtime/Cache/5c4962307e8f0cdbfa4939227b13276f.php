...
<div class="container">
	...
	<div class="row">		
		<div class="span12">
			...
			<form id="form1" action="<?php echo U('leads/edit');?>" method="post">
				<input type="hidden" name="leads_id" id="leads_id" value="<?php echo ($leads["leads_id"]); ?>"/>
				<table class="table" width="95%" border="0" cellspacing="1" cellpadding="0">
					<tfoot>
						<tr>
							<td>&nbsp;</td>
							<td <?php if(C('ismobile') != 1): ?>colspan="3"<?php endif; ?>><input name="submit" class="btn btn-primary" type="submit" value="<?php echo L('SAVE');?>"/>  &nbsp;<input class="btn" type="button" onclick="javascript:history.go(-1)"  value="<?php echo L('RETURN');?>"/></td>						
						</tr>
					</tfoot> 
					<tbody width="100%">
						<tr><th colspan="4"><?php echo L('MIAN_INFO');?></th></tr>
						<tr>
							<td class="tdleft"><?php echo L('LEADS_OWNER');?></td>
							<td><input type="hidden" id="owner_id" name="owner_role_id" value="<?php echo ($leads["owner_role_id"]); ?>"/><input type="text" id="owner_name" value="<?php echo ($leads["owner"]["user_name"]); ?>" name="owner_name"/> &nbsp;&nbsp;<input class="btn btn-mini" id="remove"  type="button" value="<?php echo L('PUT_LEADS_INTO_THE_POOL');?>"/> &nbsp;<a target="_blank" href="http://www.5kcrm.com/index.php?m=question&a=view&id=12"><?php echo L('WHAT_IS_THE_LEADS_POOL');?></a></td>
						<?php $j=0; ?>
...
<div id="dialog-role-list" class="hide" title="<?php echo L('CHOOSE_THE_LEADS_OWNER');?>">loading...</div>
<div class="hide" id="dialog-transform" title="<?php echo L('CONVERT_LEADS');?>">
	<form id="transform" action="<?php echo U('leads/transform');?>" method="Post">
		<input type="hidden" name="leads_id" value="<?php echo ($leads["leads_id"]); ?>"/>
		<p><?php echo L('CONVERT_TO_CUSTOMER_CONTACTS');?></br>
		&nbsp; &nbsp; <?php echo L('CUSTOMER_NAME');?>&nbsp; <?php echo ($leads["company"]); ?></br>
		&nbsp; &nbsp; <?php echo L('CONTACTS_NAME');?>&nbsp; <?php echo ($leads["name"]); ?></br>
		<span style="color:red"><?php echo L('NEED_COMPANY_CONTACTS');?></span></p>
		<h6><?php echo L('CREATE_BUSINESS_FOR_CUSTOMER');?></h6>
		</p>
		<div class="" id="addbusiness">
			<table class="table">
				<tr><td class="tdleft"><?php echo L('BUSINESS_NAME');?></td><td><input  type="text" name="business_name" id="name"></td></tr>
				<tr><td class="tdleft"><?php echo L('EXPECTED_DEAL_DATE');?></td><td><input  type="text"  onclick="WdatePicker()" name="due_date"/></td></tr>
				<tr><td class="tdleft"><?php echo L('BUSINESS_LEVEL');?></td>
					<td>
						<select name="status_id">
							<?php if(is_array($businessStatusList)): $i = 0; $__LIST__ = $businessStatusList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["status_id"]); ?>"><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</td>
				</tr>
				<tr><td class="tdleft"><?php echo L('PRICE');?></td><td><input type="text" id="total" name="total"/></td></tr>
			</table>
		</div>
		<?php echo L('ASSIGN_TO_SOMEONE_AFTER_CONVERTED');?><input type="hidden" id="tranform_owner_id" name="owner_role_id" value="<?php echo ($leads["owner_role_id"]); ?>"/><input type="text" id="tranform_owner_name" name="owner_name"/>
	</form>
</div>
<div class="hide" id="transform-role-list" title="<?php echo L('CHOOSE_THE_LEADS_OWNER');?>">loading...</div>
...
$(function(){
	...
	$('#validate').click(
		function(){
			name = $('#company').val();
			if(name!=''){
				$.post('<?php echo U("leads/validate");?>',
					{ 
						name:name,
					},
					function(data){
...
$(document).ready(function(){
	$("#btn_transform1").click(function(){
		...
		window.location.href='/index.php?m=customer&a=add&leads_id='+<?php echo ($leads["leads_id"]); ?>;
	});
