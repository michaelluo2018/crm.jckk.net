...
class Project extends Model{
    ...
    public  function get_project_entity(){
        $data['product_demand'] = config("product_demand");//产品需求
        $data['contract_status'] = config("contract_status");
        $data['payment_type'] = config("payment_type");
        $data['payment_status'] = config("payment_status");
        $data['project_channel'] = config("project_channel");
        $data['status_label_color'] =config("status_label_color");
        return $data;
    }
    ...
    public  function  save_project($data){
        ...
        if(isset($data['project_id'])){
            ...
            if($project =model("project","model")->where(["project_name"=>$project_name , "customer_id"=>$customer_id , "is_delete"=>1])->find()){
                ...
                $project->number = "P".date('Ymd',time())."-".rand(100,999);
            }
    ...
    public function get_projects($customer_id = null,$create_uids = null,$where=[],$keyword=''){
        ...
        if($create_uids){
            if($keyword){
                return Db::table("jckk_project")
                    ...
                    ->where("p.is_delete","<>",1)
                    ->where($where)
                    ->where("p.project_name|c.customer_name|p.company_name","like","%".$keyword."%")
    ...
    public function get_projects_by_status($key,$name,$create_uids=null,$where=[],$keyword=''){
    ...
    //获取项目列表
    public function project_recycle($customer_id = null,$create_uids=null,$where=[],$keyword=''){
        if($customer_id){
            $where["p.customer_id"] = $customer_id;
        }
