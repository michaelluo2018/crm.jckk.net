...
class Customer extends  Model{
    ...
    public function get_customers($create_uids = null,$where=[],$keyword=""){
        if($create_uids) {
            if($keyword){
                return Db::table("jckk_customer")
                    ->where("jckk_customer.is_delete", "<>", 1)
                    ->where($where)
                    ->where("jckk_customer.customer_name|jckk_customer.number|jckk_contact.mobile|jckk_contact.contact_name","like","%".$keyword."%")
    ...
    public function get_customers_by_status($key,$status,$create_uids=null,$where=[],$keyword=""){
        if($create_uids){
            if($keyword){
                return Db::table("jckk_customer")
                    ->where("jckk_customer.is_delete","<>",1)
